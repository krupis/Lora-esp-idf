...
int lora_init(void)
{
   ...
   gpio_config_t lora_rst_config =
       {
           .mode = GPIO_MODE_OUTPUT,
           .pin_bit_mask = 1ULL << LORA_RST_PIN};
   ESP_ERROR_CHECK(gpio_config(&lora_rst_config));

   // gpio_pad_select_gpio(LORA_CS_PIN);
   gpio_config_t lora_cs_config =
       {
           .mode = GPIO_MODE_OUTPUT,
           .pin_bit_mask = 1ULL << LORA_CS_PIN};
   ESP_ERROR_CHECK(gpio_config(&lora_cs_config));

   spi_bus_config_t bus = {
       .miso_io_num = LORA_MISOx_PIN,
